...
- **Rendering**: the plan is an SVG string, a `computed` fed to `v-html`, so the same code serves interactive view and export. SVG units are cm; `mkK(scale)` converts screen px to cm for strokes and fonts. SVG colors come from `PAL.light/dark` (via `palette`), not CSS vars. Export matches the current theme (same `palette`) and the same default label sizes as on-screen; only its raster scale differs, picked so the room fits a ~1600px image (see `features/export-layout`).
